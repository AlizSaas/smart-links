...
import { linksQueryOptions } from "@/data/links/links-fetch-query";

export const Route = createFileRoute("/app/_authed/links")({
  component: RouteComponent,
  pendingComponent: SkeletonRow,
  loader: async ({ context }) => {
    await context.queryClient.ensureQueryData(linksQueryOptions())
  },
});
    ...
    cell: (info) => {
      ...
      return (
        <div className="flex items-center gap-2">
          <span className="truncate max-w-50">{`https://${import.meta.env.VITE_BACKEND_HOST}/${info.getValue()}`}</span>
          <Button
            variant="ghost"
            size="sm"
            onClick={(e) => {
...
function SkeletonRow() {
  return  <div>loading the links lol</div>
}
