...
import { createLink } from "@/data/links/link-functions";
...
function RouteComponent() {
  ...
  const createMutation = useMutation({
    mutationFn: async (variables: CreateLinkSchemaType) => {
      return await createLink({ data: variables });
    },
    onSuccess: (result) => {
      toast.success("Link created successfully!");
      // Invalidate links query to refetch the list
      queryClient.invalidateQueries({ queryKey: ["links"] });
      navigate({ to: "/app/links" });
    },
    onError: (error) => {
