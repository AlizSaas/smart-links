	...
	const result = await generateObject({
		...
		schema: z.object({
            pageStatus: z.object({
                ...
                // 1. Move description here using .describe()
                .describe(`
                    Indicates the product's availability on the page:
                    - AVAILABLE_PRODUCT: The product appears available for purchase.
                    - NOT_AVAILABLE_PRODUCT: The product appears unavailable.
                    - UNKNOWN_STATUS: The status could not be determined.
                `.trim()),

                statusReason: z.string().describe(
                    'A concise explanation citing specific words...'
                ),
            })
            .describe('Information about the product availability status...'),
        })
