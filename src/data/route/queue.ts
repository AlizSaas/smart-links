...
export async function captureLinkClickInBackground(env: Env, event: LinkClickMessageType) {
  ...
  // Only update DO for real-time WebSocket broadcast if we have complete geo data
  // Type guard ensures latitude/longitude/country are defined
  if (latitude !== undefined && longitude !== undefined && country !== undefined) {
