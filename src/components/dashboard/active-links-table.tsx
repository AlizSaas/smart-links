...
import { activeLinksQueryOptions } from "@/data/links/links-fetch-query";
import { formatRelativeTime } from "@/lib/utils";

export function ActiveLinksTable() {
  const navigate = useNavigate();

  const { data } = useSuspenseQuery(activeLinksQueryOptions());
              ...
              {data.map((link, index) => (
                ...
                >
                  ...
                  <TableCell className="text-right text-muted-foreground">
                    {link.lastClicked
                      ? formatRelativeTime(link.lastClicked.toISOString())
                      : "-"}
                  </TableCell>
                </TableRow>
              ))}
